            ...
            <div className="flex space-x-4">
                {
                    props.items.map(item =>
                        <a href={item.route} aria-current="page"
                            className="text-gray-300 px-3 py-2 rounded-md text-lg font-medium
    ...
    const MobileLinkMenu =
        <div className="sm:hidden" id="mobile-menu">
            <div className="space-y-8 px-4 pt-4 pb-4 text-center">
                {
                    props.items.map(item =>
                        <a href={item.route} onClick={props.setIsClosed} aria-current="page"
                            className="bg-teal-600 text-gray-100 text-xl font-sans block px-3 py-4 rounded-md text-base transition
                            focus:duration-200 focus:delay-100 focus:bg-slate-50 focus:ring-2 focus:ring-offset-4 focus:ring-offset-teal-700 focus:ring-white focus:text-black">{item.name}</a>
                    )
                }
            </div>
        </div>;
